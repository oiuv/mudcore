...
private string _mudcore_impl_get_name() { return "树林(热带)[9格], 樣式隨機 "; }
...
private mapping get_icon() {
    mapping icon = ([
        0: ({ get_sample(), get_sample(), get_sample() }),
        1: ({ get_sample(), get_sample(), get_sample() }),
        2: ({ get_sample(), get_sample(), get_sample() }),
    ]);
