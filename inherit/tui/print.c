...
varargs string p_progress(int pct, int width, mapping opts) {
    ...
    return repeat_string("█", filled) + repeat_string("░", width - filled) + sprintf(" %3d%%", pct);
}
...
varargs string p_panel(string title, string body, mapping opts) {
    ...
    out = chars[0] + chars[9] + title + repeat_string(
        chars[9],
        inner - visible_width(title) + 1
    ) + chars[1] + "\n";
    foreach (string l in lines) {
        out += chars[10] + " " + l + repeat_string(
            " ",
            inner - visible_width(l)
        ) + " " + chars[10] + "\n";
    }
...
varargs string p_header(string text, int width, string attr) {
    ...
    return ansi_sgr(attr) + repeat_string(
        " ",
        pad
    ) + text + repeat_string(" ", width - pad - visible_width(text)) + ansi_reset() + "\n";
}
...
varargs string p_bigtext(string text, string fcolor) {
    ...
    if (catch(big = (string)CORE_SIMUL_EFUN_OB->bitmap_font(text, 12, "█", " ", fcolor, ""))) {
        return text + "\n";
    }
