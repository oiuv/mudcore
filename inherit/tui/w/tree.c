    ...
    switch (ev) {
        ...
        case TUI_KEY_LEFT:
            if (v[3] && expanded[v[0]]) {
                ...
                // leaf or already collapsed: jump to the parent node
                string parent = v[0][0..strsrch(v[0], "/", -1) - 1];
                int i;

                for (i = 0; i < sizeof(visible); i++) {
                    if (visible[i][0] == parent) {
                        sel = i;
                        emit_event("change", visible[sel][2]);
                        break;
                    }
...
int handle_mouse(mixed *ev) {
    ...
    if (!ev[4] || !sizeof(visible)) return 0;
    if (b == 64 || b == 65) {  // wheel scrolls the selection
        int ns = sel + (b == 64 ? -1 : 1);

        if (ns >= 0 && ns < sizeof(visible)) {
            sel = ns;
            emit_event("change", visible[sel][2]);
        }
        return 1;
    }
    if (b < 3 && y >= wy && y < wy + wh && top + y - wy < sizeof(visible)) {
        sel = top + y - wy;
        emit_event("change", visible[sel][2]);
        return 1;
    }
