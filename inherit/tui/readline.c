...
private mixed *undo_stack = ({});  // ({ buf, pos }) snapshots, oldest first
#define UNDO_MAX 200
...
private string isearch_prompt() {
    return (is_fail ? "(failed " : "(") + (is_dir < 0 ? "reverse-i-search" : "i-search") + ")`" + is_q + "': ";
}
...
private void render() {
    ...
    {
        // scroll markers: "<" replaces the first visible column when scrolled,
        // ">" the last when there is more text to the right — independently,
        // so a mid-line viewport shows both.
        int lm = scroll > 0;
        int rm = total - scroll > avail;

        view = (lm ? "<" : "") + wslice(disp, scroll + lm, avail - lm - rm) + (rm ? ">" : "");
    }
...
private void isearch_feed(mixed ev) {
    if (arrayp(ev)) return;  // mouse/CPR reports don't cancel the search
    if (stringp(ev)) {
        is_q += ev;
        isearch_find(0);
...
private void complete() {
    ...
    if (lcp > strlen(frag)) {
        ...
        // the fragment already IS the unique match: just terminate the word
        insert(" ");
...
private void kb_undo() {
    mixed *snap;

    if (!sizeof(undo_stack)) return;
    snap = undo_stack[<1];
    undo_stack = undo_stack[0..<2];
    buf = snap[0];
    pos = snap[1];
}
...
private void build_keymap() {
    ...
    // undo restores a snapshot instead of taking one; dispatch special-cases it
    keymap[TUI_CTRL('_')] = (: kb_undo :);
}
...
private void dispatch(mixed ev) {
    ...
    if (!is_undo && buf != obuf) {  // buffer changed: snapshot for C-_
        undo_stack += ({ ({ obuf, opos }) });
        if (sizeof(undo_stack) > UNDO_MAX) undo_stack = undo_stack[1..];
    }
