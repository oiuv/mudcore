...
`CORE_TUI_*` 始终指向框架实现；`TUI_*` 默认指向同名实现，宿主可在包含 `<tui.h>` 之前覆盖。`CORE_DIR` 重定位也适用。框架生产源码使用 `.c`，对象路径省略扩展名。
...
- 中文、退格和方向键的完整支持依赖 2026 年 FluffOS 对字符模式 UTF-8、控制键、ESC 及分包处理的修复；接口存在不等于这些修复已经包含，旧驱动应实际验证。
