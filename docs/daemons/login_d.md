...
内部创建收尾再次验证登录对象及私有认证状态，再调用 `make_body()`、`init_new_player()`、`enter_world()`；不能从外部调用策略或伪造临时属性获得身份。失败清理连接/玩家对象、恢复 daemon 身份，并通过 `NAME_D->assure_map_name(name)` 核对离线存档、调用 `save()` 持久化校验结果：未保存的角色释放姓名，已保存但入场失败的角色保留姓名占用。替换 `NAME_D` 时需保留这两个接口；清理后仍上报原始异常。这不是跨多个存档和姓名索引的原子事务。初始属性仍交给 `CHAR_D`；保存/重连时不重复调用新角色初始化。
