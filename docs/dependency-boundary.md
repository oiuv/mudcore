...
本文记录 `2.0.0-dev` 开发版本相对旧实现的兼容性变更。初次接入见 [接入指南](integration.md)，升级和正式发布步骤见 [版本管理](maintenance.md)。
...
默认 `CORE_LIVING`、`CORE_USER`、`_USER` 继承组合及存档字段不变；新增 `CORE_USER_BASE/_USER_BASE` 由宿主显式选用。非 RPG 宿主还需选择命令目录/阶段、关闭不使用的 parser，并通过 `CHAR_D` 初始化自己的属性，见 [接入示例](integration.md#显式选择最小玩家组合)。已有玩家切换组合需宿主评估存档，不自动删除字段或迁移数据。

`MUDCORE_ENABLE_PARSER` 默认 `1`；设为 `0` 后，默认命令与登录不初始化 parser 或重载 `VERB_D`。宿主显式选择不可用 parser 会报错。表情/频道通过 `_COMMAND` 的阶段钩子选择；已选服务的错误不会被忽略。移动只在对应协作者存在时调用 GMCP 或可选 DB 存档，保留已选组件原有行为及错误传播。

登录新增局部名称/性别策略钩子，默认中文名称和 m/f 含义不变，认证与 UID 授权未放宽。`_HOST_POLICY` 是按需只读来源诊断，不能证明 ACL 安全；框架不据此自动限制 HTTP/Socket 等通用能力。扩展约束与详细签名见 [架构规范](architecture.md) 和 [模块契约](module-contracts.md)。
...
老 MUD 仍可按需集成，无需整体替换：升级驱动后使用后续框架版本，或保留旧驱动并将框架固定到上述提交或更早的提交。公共 `CORE_*` 宏、原有继承路径、历史别名和存档兼容约定不因此取消，详见 [驱动要求](integration.md#源文件与驱动能力)。

新建 LPC 源码优先 `.lpc`，已有 `.c` 文件本次不改名。扫描仍识别两种扩展名，同名优先 `.lpc`，但项目不应同时维护同名双扩展文件。`env()`、中文转换和时间辅助函数继续尊重宿主 daemon 别名覆盖。
