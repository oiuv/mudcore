// Adapted from FluffOS testsuite/std/decimal.lpc.
// Upstream license: UPSTREAM-COPYRIGHT (also applies to the imported tests).
 ...
 *
 * Included by the framework simul_efun so to_decimal()/decimal_*() are global
 * simul-efuns. Tested by mudcore/tests/lpc/decimal.lpc.
 */
...
int *decimal_div(int *a, int *b) {
    ...
    num = shift >= 0 ? _mul_checked(a[DEC_MANT], _pow10(shift)) : a[DEC_MANT] / _pow10(-shift);
    q = num / b[DEC_MANT];
...
string decimal_to_string(int *d) {
    string digits, out;
    int mant = d[DEC_MANT], scale = d[DEC_SCALE], neg = d[DEC_MANT] < 0, intlen, t, dg;
